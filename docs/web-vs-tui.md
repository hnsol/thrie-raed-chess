...
| 序盤の定跡戦略3択 | — | 対局開始時に戦略選択（イタリアン/ロンドン/フィアンケット流）。序盤の間、定跡手（loss≤30時のみ）をgreen候補に採用しコーチも言及 | web独自の学習機能（lib/openings.ts） |

## 運用メモ

**凍結時点のスナップショット（2026-07-19）**

- 新機能・仕様変更は **web/（TypeScript）に実装する**
- TUI版（`thrie_raed_chess/`）は現状凍結。バグ修正のみ受け付ける
- この対応表は凍結時点のスナップショット。以降の差分は web/ 側のみに蓄積する
- パズルデータの正は引き続き `thrie_raed_chess/data/`。web側はビルド時コピー（gitignore）
- コーチのフレーズ表を変更する場合は web/src/lib/coach.ts のみを編集する
